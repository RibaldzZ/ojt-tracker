...
function buildCredentials() {
  ...
  // Option 2: Individual env vars (raw key without PEM headers)
  if (process.env.GCP_PRIVATE_KEY && process.env.GCP_CLIENT_EMAIL) {
    let pk = process.env.GCP_PRIVATE_KEY;

    // The env var has raw key content (no PEM headers), so we need to wrap it
    // Handle escaped newlines (\\n) -> actual newlines
    if (pk.includes("\\n")) pk = pk.replace(/\\n/g, "\n");
    // Normalize line endings
    pk = pk.replace(/\r\n/g, "\n");
    // Remove any existing PEM wrappers that might exist
    ...
    console.error("PK wrapped: starts with", pk.substring(0, 35));
...
async function getAccessToken() {
  ...
  try {
    ...
    console.error("Step 1 OK: createPrivateKey");

    // Step 2: Build JWT header & claim
    ...
    console.error("Step 2 OK: JWT header built");
    ...
    console.error("Step 3 OK: Signature created");
    ...
    console.error("Step 4 OK: JWT assembled, length=" + jwt.length);
    ...
    console.error("Step 5 OK: Token obtained");
    cachedToken = tokenData.access_token;
    tokenExpiry = Date.now() + tokenData.expires_in * 1000;
    return cachedToken;
  } catch (e) {
    console.error("getAccessToken FAILED at:", e.message);
    console.error("Key preview:", private_key.substring(0, 50) + "...");
    console.error("Key length:", private_key.length);
    throw e;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let procPk = "N/A";
    try { procPk = buildCredentials().private_key; } catch (e) { procPk = "Build failed: " + e.message; }
    return res.status(500).json({
      error: error.message,
      procPkPreview: procPk.substring(0, 60),
      procPkEnd: procPk.substring(procPk.length - 40),
      procPkLen: procPk.length,
      procPkNewlines: (procPk.match(/\n/g) || []).length,
    });
    });
  }
}
